...
Скрипт на Python, который собирает названия, авторов и рейтинги
русскоязычных детективов с сайта [livelib.ru](https://www.livelib.ru) и
сохраняет их в CSV-файл (открывается в Excel/Google Sheets).

Livelib — сайт отзывов и рейтингов книг (не интернет-магазин), поэтому цены
там физически нет — она подгружается отдельным запросом только по клику на
кнопку "Купить". Честно отмечаем это в колонке "цена" вместо того, чтобы
что-то придумывать.
...
2. С каждой карточки книги забирает: название, автора, рейтинг.
...
Столбцы: `название`, `автор`, `рейтинг`, `цена`, `источник`.
